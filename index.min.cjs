...
var BELAMessage = class {
  static say(data = {
    ...
    const boldRegex = /\*(.*?)\*/g;
    const formattedMessage = data.message.replace(underlineRegex, (_, text) => import_chalk.default.underline(text)).replace(boldRegex, (_, text) => import_chalk.default.bold(text));
    const formattedStack = data.stack ? data.stack.replace(/[•\-–—]/g, (match) => import_chalk.default.grey(match)).replace(underlineRegex, (_, text) => import_chalk.default.underline(text)).replace(boldRegex, (_, text) => import_chalk.default.bold(text)) + `
  ${import_chalk.default.grey("\u2022")} ${data.name} ${import_chalk.default.bold.underline(data.version ?? version)}
` : `  ${import_chalk.default.grey("\u2022")} ${data.name} ${import_chalk.default.bold.underline(data.version ?? version)}
`;
    if (data.type === "error") {
      console.log(
        `${import_chalk.default.cyan(data.name)} - ${import_chalk.default.red(data.type)} ${import_chalk.default.grey(data.code + ":")} ${formattedMessage}
${formattedStack}`
      );
      return "";
      process.exit(1);
      ...
      console.log(
        `${import_chalk.default.cyan(data.name)} - ${import_chalk.default.green(data.type)} ${import_chalk.default.grey(data.code + ":")} ${formattedMessage}
${formattedStack}`
      );
      return "";
      ...
      return `${import_chalk.default.cyan(data.name)} - ${import_chalk.default.yellow("warning")} ${import_chalk.default.grey(data.code + ":")} ${formattedMessage}`;
    } else {
...
var IV2 = import_crypto.default.randomBytes(16);
var KEY2;
function decrypt(encryptedText) {
  try {
    const [ivHex, encrypted] = encryptedText.split(":");
    const iv = Buffer.from(ivHex, "hex");
    const decipher = import_crypto.default.createDecipheriv(ALGORITHM2, KEY2, iv);
...
var BELA = class {
  isModel = false;
  packageRoot = path3.dirname(require.main?.filename ?? process.cwd());
  /** Conversation */
  conversationTrainer;
